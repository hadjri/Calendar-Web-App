...
if (validCSRF()){
    ...
    if ($title != ""){
        $cstmt = $mysqli->prepare("update events set title = ?, flag = ?, datetime = ? where event_id = ? AND username = ?"); //make sure the user owns that event
        $cstmt->bind_param("sisis", $title, $flag, $_POST["datetime"], $_POST["event_id"],$_SESSION['username']);
         if(!$cstmt){
